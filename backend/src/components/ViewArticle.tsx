...
//   title: string;
//   author?: string;
//   hero_image?: string; // filename only
//   introduction?: string;
//   sections?: { title?: string; content?: string }[];
//   faqs?: { question: string; answer?: string }[];
//   tools?: { name: string; url?: string }[];
...
//   const navigate = useNavigate();
//   const { getOne } = useCommonCrud<Article>({ role: role || "admin", module: "article" });
//   const [article, setArticle] = useState<Article | null>(null);
//   const [loading, setLoading] = useState(true);
...
//   const getImageUrl = (filename?: string, folder: string = "article") => {
//     if (!filename) return "/no-image.png"; // fallback image
//     const base = API_BASE_URL.replace("/api", "");
...
//           <div className="mt-6">
//             <img
//               src={getImageUrl(article.hero_image, "hero")}
//               alt={article.title}
//               className="w-full h-auto max-h-[590px] object-cover border-t border-b"
//               onError={(e) => ((e.currentTarget.src = "/no-image.png"))}
//             />
...
//               {article.sections.map((sec, idx) => (
//                 <div key={idx}>
//                   {sec.title && <h2 className="text-2xl font-semibold text-gray-800 mb-3">{sec.title}</h2>}
//                   {sec.content && (
//                     <div
//                       className="prose max-w-none prose-headings:text-gray-900 prose-p:text-gray-700 prose-img:rounded-lg prose-img:shadow-lg prose-a:text-indigo-600 hover:prose-a:text-indigo-800"
//                       dangerouslySetInnerHTML={{
//                         __html: sec.content.replace(
//                           /uploads\/article\/([a-zA-Z0-9\-\_]+\.(jpg|jpeg|png|webp|gif))/g,
//                           (_match, filename) => getImageUrl(filename, "article")
//                         ),
//                       }}
//                     />
//                   )}
...
//                   <div key={idx} className="border-b pb-4">
//                     <h3 className="font-semibold text-gray-800">{faq.question}</h3>
//                     <div className="text-gray-700 mt-2 prose" dangerouslySetInnerHTML={{ __html: faq.answer || "" }} />
//                   </div>
//                 ))}
//               </div>
//             </div>
...
//   const { id, role } = useParams<{ id: string; role?: string }>();
//   const navigate = useNavigate();
//   const { getOne } = useCommonCrud({ role: role || "admin", module: "article" });
//   const [article, setArticle] = useState<Article | null>(null);
//   const [loading, setLoading] = useState(true);
...
//         // Cast response to Article since getOne returns {} by default
//         const response = (await getOne(id)) as Article;
...
//             related_reads: response.related_reads || [],
//           });
//         } else {
//           toast.error("Article not found or deleted.");
//         }
...
export default function ViewArticle() {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      try {
        // Correctly extract the data from the API response
        const response = await getOne(id); // response: ApiResponse<Article>
        const articleData: Article | null = response?.data ?? null;

        if (articleData?._id || articleData?.title) {
          setArticle({
            ...articleData,
            sections: articleData.sections || [],
            faqs: articleData.faqs || [],
            tools: articleData.tools || [],
            related_reads: articleData.related_reads || [],
          });
        } else {
          toast.error("Article not found or deleted.");
        }
    ...
    <div className="bg-gray-50 min-h-screen py-6">
      <div className="max-w-5xl mx-auto bg-white shadow-md overflow-hidden rounded-lg">
        ...
        <div className="px-6 py-10 md:px-12">
          ...
          {article.faqs.length > 0 &&
            article.faqs.map((faq, idx) => (
              <div key={idx} className="border-b pb-4 mt-6">
                <h3 className="font-semibold text-gray-800">{faq.question}</h3>
                <div className="text-gray-700 mt-2 prose" dangerouslySetInnerHTML={{ __html: faq.answer || "" }} />
              </div>
            ))}
