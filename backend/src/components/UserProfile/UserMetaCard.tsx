...
//   const backendUrl = import.meta.env.VITE_API_BASE; // Example: http://localhost:5000/api

//   // Reset Form when user data changes
//   const resetForm = () => {
//     if (!user) return;

//     const fullImagePath = user.profileImage
//       ? `${backendUrl}/${user.profileImage.replace(/^\/+/, "")}?v=${Date.now()}`
...
//         setImagePreview(`${backendUrl}/${res.data.user.profileImage.replace(/^\/+/, "")}?v=${Date.now()}`);
//       }
...
//           if (e.param === "firstname") fieldErrors.firstname = e.msg;
//           if (e.param === "lastname") fieldErrors.lastname = e.msg;
...
export default function UserMetaCard() {
  ...
  const resetForm = () => {
    ...
    const fullImagePath = user.profileImage
      ? `${backendUrl}${user.profileImage.startsWith("/") ? "" : "/"}${user.profileImage}?v=${Date.now()}`
      : "/images/user/owner.jpg";
